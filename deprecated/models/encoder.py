...
class EfficientNetB7(nn.Module):
    def __init__(self):
        ...
        self.model = efficientnet_b7(weight=EfficientNet_B7_Weights.DEFAULT)
        
        # output shape -> [B, 2560, 15, 15]
        self.feature_extractor = IntermediateLayerGetter(self.model, return_layers={"features": "feature"})
        self.output_channel = 2560
        self.output_size = None
    ...
    def forward(self, x):
        ...
        # B x C x (H * W)
        pos_emb = embedding.positional_encoding_2d(out.shape[2], out.shape[3], out.shape[1]).to(dtype=out.dtype, device=out.device)
